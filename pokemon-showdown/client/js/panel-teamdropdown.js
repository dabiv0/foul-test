...
var plus='',minus='';for(var _i2=0;_i2<
evLines.length;_i2++){var evLine=evLines[_i2];
evLine=evLine.trim();
...
var nature=this.getNature(plus,minus);
if(nature!=='Serious'){
set.nature=nature;
}
}else if(line.startsWith('IVs: ')){
var ivLines=line.slice(5).split(' / ');
set.ivs={hp:31,atk:31,def:31,spa:31,spd:31,spe:31};for(var _i4=0;_i4<
ivLines.length;_i4++){var ivLine=ivLines[_i4];
ivLine=ivLine.trim();
...
getNature=function getNature(plus,minus){
if(!plus||!minus){
return'Serious';
}
...
lines.length;_i6++){var line=lines[_i6];
line=line.trim();
if(line===''||line==='---'){
curSet=null;
...
while(lines.length&&!lines[0])lines.shift();
while(lines.length&&!lines[lines.length-1])lines.pop();for(var _i8=0;_i8<

lines.length;_i8++){var line=lines[_i8];
line=line.trim();
if(line===''||line==='---'){
curSet=null;
}else if(line.startsWith('===')){
...
var teamBuckets={};for(var _i10=0,_teams2=
teams;_i10<_teams2.length;_i10++){var team=_teams2[_i10];
...
var genList=[];for(var _i12=0,_PS$teams$list2=
PS.teams.list;_i12<_PS$teams$list2.length;_i12++){var _team=_PS$teams$list2[_i12];
...
var searchID=toID(this.search);for(var _i14=0;_i14<
formats.length;_i14++){var format=formats[_i14];
