...
var battle_animations_exports = {};
__export(battle_animations_exports, {
  BattleOtherAnims: () => BattleOtherAnims,
  BattleScene: () => BattleScene,
  BattleStatusAnims: () => BattleStatusAnims,
  PokemonSprite: () => PokemonSprite,
  Sprite: () => Sprite
});
module.exports = __toCommonJS(battle_animations_exports);
var import_battle_animations_moves = require("./battle-animations-moves");
var import_battle_log = require("./battle-log");
var import_battle_sound = require("./battle-sound");
...
class BattleScene {
  constructor(battle, $frame, $logFrame) {
    ...
    this.$frame = $frame;
    this.log = new import_battle_log.BattleLog($logFrame[0], this);
    this.log.battleParser.pokemonName = (pokemonId) => {
      ...
      if (pokemonId.charAt(3) === ":") return import_battle_text_parser.BattleTextParser.escapeReplace(pokemonId.slice(4).trim());
      else if (pokemonId.charAt(2) === ":") return import_battle_text_parser.BattleTextParser.escapeReplace(pokemonId.slice(3).trim());
    ...
    if (!numericId) {
      numericId = Math.floor(Math.random() * 1e6);
    }
    this.numericId = numericId;
    this.tooltips = new import_battle_tooltips.BattleTooltips(battle);
  ...
  reset() {
    ...
    this.$bg = $('<div class="backdrop" style="background-image:url(' + import_battle_dex.Dex.resourcePrefix + this.backdropImage + ');display:block;opacity:0.8"></div>');
  ...
  setMute(muted) {
    import_battle_sound.BattleSound.setMute(muted);
  }
  wait(time) {
    if (!this.animating) return;
  ...
  pos(loc, obj) {
    ...
    let left = 210;
    let top = 245;
    let scale = obj.gen === 5 ? 2 - loc.z / 200 : 1.5 - 0.5 * (loc.z / 200);
  ...
  finishAnimations() {
    ...
    if (!this.activeAnimations.length) return void 0;
  ...
  runMoveAnim(moveid, participants) {
    if (!this.animating) return;
    let animEntry = import_battle_animations_moves.BattleMoveAnims[moveid];
    if (this.acceleration >= 3) {
      ...
      animEntry = import_battle_animations_moves.BattleMoveAnims["tackle"];
    }
  ...
  runResidualAnim(moveid, pokemon) {
    if (!this.animating) return;
    import_battle_animations_moves.BattleMoveAnims[moveid].residualAnim(this, [pokemon.sprite]);
  }
  runPrepareAnim(moveid, attacker, defender) {
    if (!this.animating || this.acceleration >= 3) return;
    const moveAnim = import_battle_animations_moves.BattleMoveAnims[moveid];
    if (!moveAnim.prepareAnim) return;
    moveAnim.prepareAnim(this, [attacker.sprite, defender.sprite]);
  }
  updateGen() {
    let gen = this.battle.gen;
    if (import_battle_dex.Dex.prefs("nopastgens")) gen = 6;
    if (import_battle_dex.Dex.prefs("bwgfx") && gen > 5) gen = 5;
    this.gen = gen;
    ...
    if (this.$bg) {
      this.$bg.css("background-image", `url(${import_battle_dex.Dex.resourcePrefix}${this.backdropImage})`);
    }
  }
  getDetailsText(pokemon) {
    ...
    return import_battle_log.BattleLog.escapeHTML(name);
  }
  getSidebarHTML(side, posStr) {
    ...
    for (let i = 0; i < sidebarIcons.length; i++) {
      ...
      if (iconType === "empty") {
        pokemonhtml += `<span class="picon" style="${import_battle_dex.Dex.getPokemonIcon("pokeball-none")}"></span>`;
      } else if (noShow) {
        if (poke?.fainted) {
          pokemonhtml += `<span${tooltipCode} style="${import_battle_dex.Dex.getPokemonIcon("pokeball-fainted")}" aria-label="Fainted"></span>`;
        } else if (poke?.status) {
          pokemonhtml += `<span${tooltipCode} style="${import_battle_dex.Dex.getPokemonIcon("pokeball-statused")}" aria-label="Statused"></span>`;
        } else {
          pokemonhtml += `<span${tooltipCode} style="${import_battle_dex.Dex.getPokemonIcon("pokeball")}" aria-label="Non-statused"></span>`;
        }
      } else if (iconType === "pseudo-zoroark") {
        pokemonhtml += `<span class="picon" style="${import_battle_dex.Dex.getPokemonIcon("zoroark")}" title="Unrevealed Illusion user" aria-label="Unrevealed Illusion user"></span>`;
      } else if (!poke) {
        pokemonhtml += `<span class="picon" style="${import_battle_dex.Dex.getPokemonIcon("pokeball")}" title="Not revealed" aria-label="Not revealed"></span>`;
        ...
        pokemonhtml += `<span${tooltipCode} style="${import_battle_dex.Dex.getPokemonIcon(poke, !side.isFar)};opacity:0.6" aria-label="${details}"></span>`;
      } else {
        const details = this.getDetailsText(poke);
        pokemonhtml += `<span${tooltipCode} style="${import_battle_dex.Dex.getPokemonIcon(poke, !side.isFar)}" aria-label="${details}"></span>`;
      }
      if (i % 3 === 2) pokemonhtml += `</div><div class="teamicons">`;
    }
    pokemonhtml = '<div class="teamicons">' + pokemonhtml + "</div>";
    const ratinghtml = side.rating ? ` title="Rating: ${import_battle_log.BattleLog.escapeHTML(side.rating)}"` : ``;
    ...
    if (side.badges.length) {
      ...
      for (const badgeData of side.badges.slice(0, 3)) {
        ...
        badgehtml += `<img src="${import_battle_dex.Dex.resourcePrefix}/sprites/misc/${formatType}_${type}.png" style="padding: 0px 1px 0px 1px" width="16px" height="16px" title="${hover}" />`;
      }
      badgehtml += "</span>";
    }
    return `<div class="trainer trainer-${posStr}"${faded}><strong>${import_battle_log.BattleLog.escapeHTML(side.name)}</strong><div class="trainersprite"${ratinghtml} style="background-image:url(${import_battle_dex.Dex.resolveAvatar(side.avatar)})"></div>${badgehtml}${pokemonhtml}</div>`;
  }
  updateSidebar(side) {
  ...
  teamPreview() {
    ...
    for (let siden = 0; siden < 2 || this.battle.gameType === "multi" && siden < 4; siden++) {
      ...
      for (let i = 0; i < side.pokemon.length; i++) {
        ...
        let spriteData = import_battle_dex.Dex.getSpriteData(pokemon, !!spriteIndex, {
          gen: this.gen,
          noScale: true,
          mod: this.mod
        });
        ...
        if (gender === "M" || gender === "F") {
          buf2 += `<img src="${import_battle_dex.Dex.fxPrefix}gender-${gender.toLowerCase()}.png" alt="${gender}" width="7" height="10" class="pixelated" style="margin-bottom:-1px" /> `;
        }
        ...
        if (pokemon.item === "(mail)") {
          buf2 += ` <img src="${import_battle_dex.Dex.resourcePrefix}fx/mail.png" width="8" height="10" alt="F" style="margin-bottom:-1px" />`;
        } else if (pokemon.item) {
          buf2 += ` <img src="${import_battle_dex.Dex.resourcePrefix}fx/item.png" width="8" height="10" alt="F" style="margin-bottom:-1px" />`;
        }
        buf2 += "</div>";
      }
      ...
      if (textBuf) {
        this.log.addDiv(
          "chat battle-history",
          `<strong>${import_battle_log.BattleLog.escapeHTML(side.name)}'s team:</strong> <em style="color:#445566;display:block;">${import_battle_log.BattleLog.escapeHTML(textBuf)}</em>`
        );
      }
  ...
  pseudoWeatherLeft(pWeather) {
    let buf = `<br />${import_battle_dex.Dex.moves.get(pWeather[0]).name}`;
  ...
  sideConditionLeft(cond, isFoe, all) {
    ...
    let buf = `<br />${isFoe && !all ? "Foe's " : ""}${import_battle_dex.Dex.moves.get(cond[0]).name}`;
    if (this.battle.gen < 7 && this.battle.hardcoreMode) return buf;
  ...
  updateWeather(instant) {
    ...
    for (const pseudoWeatherData of this.battle.pseudoWeather) {
      terrain = (0, import_battle_dex.toID)(pseudoWeatherData[0]);
    }
  ...
  addPokemonSprite(pokemon) {
    const sprite = new PokemonSprite(import_battle_dex.Dex.getSpriteData(pokemon, pokemon.side.isFar, {
      gen: this.gen,
      mod: this.mod
    }), {
      x: pokemon.side.x,
  ...
  typeAnim(pokemon, types) {
    const result = import_battle_log.BattleLog.escapeHTML(types).split("/").map(
      (type) => `<img src="${import_battle_dex.Dex.resourcePrefix}sprites/types/${encodeURIComponent(type)}.png" alt="${type}" class="pixelated" />`
    ).join(" ");
  ...
  preloadEffects() {
    ...
    this.preloadImage(import_battle_dex.Dex.resourcePrefix + "sprites/ani/substitute.gif");
    this.preloadImage(import_battle_dex.Dex.resourcePrefix + "sprites/ani-back/substitute.gif");
  }
  rollBgm() {
  ...
  setBgm(bgmNum) {
    ...
    switch (bgmNum) {
      case -1:
        this.bgm = import_battle_sound.BattleSound.loadBgm("audio/bw2-homika-dogars.mp3", 1661, 68131, this.bgm);
        break;
      case -2:
        this.bgm = import_battle_sound.BattleSound.loadBgm("audio/xd-miror-b.mp3", 9e3, 57815, this.bgm);
        break;
      case -3:
        this.bgm = import_battle_sound.BattleSound.loadBgm("audio/colosseum-miror-b.mp3", 896, 47462, this.bgm);
        break;
      case 1:
        this.bgm = import_battle_sound.BattleSound.loadBgm("audio/dpp-trainer.mp3", 13440, 96959, this.bgm);
        break;
      case 2:
        this.bgm = import_battle_sound.BattleSound.loadBgm("audio/dpp-rival.mp3", 13888, 66352, this.bgm);
        break;
      case 3:
        this.bgm = import_battle_sound.BattleSound.loadBgm("audio/hgss-johto-trainer.mp3", 23731, 125086, this.bgm);
        break;
      case 4:
        this.bgm = import_battle_sound.BattleSound.loadBgm("audio/hgss-kanto-trainer.mp3", 13003, 94656, this.bgm);
        break;
      case 5:
        this.bgm = import_battle_sound.BattleSound.loadBgm("audio/bw-trainer.mp3", 14629, 110109, this.bgm);
        break;
      case 6:
        this.bgm = import_battle_sound.BattleSound.loadBgm("audio/bw-rival.mp3", 19180, 57373, this.bgm);
        break;
      case 7:
        this.bgm = import_battle_sound.BattleSound.loadBgm("audio/bw-subway-trainer.mp3", 15503, 110984, this.bgm);
        break;
      case 8:
        this.bgm = import_battle_sound.BattleSound.loadBgm("audio/bw2-kanto-gym-leader.mp3", 14626, 58986, this.bgm);
        break;
      case 9:
        this.bgm = import_battle_sound.BattleSound.loadBgm("audio/bw2-rival.mp3", 7152, 68708, this.bgm);
        break;
      case 10:
        this.bgm = import_battle_sound.BattleSound.loadBgm("audio/xy-trainer.mp3", 7802, 82469, this.bgm);
        break;
      case 11:
        this.bgm = import_battle_sound.BattleSound.loadBgm("audio/xy-rival.mp3", 7802, 58634, this.bgm);
        break;
      case 12:
        this.bgm = import_battle_sound.BattleSound.loadBgm("audio/oras-trainer.mp3", 13579, 91548, this.bgm);
        break;
      case 13:
        this.bgm = import_battle_sound.BattleSound.loadBgm("audio/oras-rival.mp3", 14303, 69149, this.bgm);
        break;
      case 14:
        this.bgm = import_battle_sound.BattleSound.loadBgm("audio/sm-trainer.mp3", 8323, 89230, this.bgm);
        break;
      case -101:
        this.bgm = import_battle_sound.BattleSound.loadBgm("audio/spl-elite4.mp3", 3962, 152509, this.bgm);
        break;
      case 15:
      default:
        this.bgm = import_battle_sound.BattleSound.loadBgm("audio/sm-rival.mp3", 11389, 62158, this.bgm);
        break;
    }
    this.updateBgm();
  }
...
class PokemonSprite extends Sprite {
  ...
  animSub(instant, noAnim) {
    if (!this.scene.animating) return;
    if (this.$sub) return;
    const subsp = import_battle_dex.Dex.getSpriteData("substitute", this.isFrontSprite, {
      gen: this.scene.gen,
      mod: this.scene.mod
    });
    this.subsp = subsp;
  ...
  reset(pokemon) {
    ...
    if (pokemon.volatiles.formechange || pokemon.volatiles.dynamax || pokemon.volatiles.terastallize) {
      ...
      this.sp = import_battle_dex.Dex.getSpriteData(pokemon, this.isFrontSprite, {
        gen: this.scene.gen,
        mod: this.scene.mod
      });
    } else if (this.oldsp) {
    ...
    for (const id in pokemon.volatiles) this.addEffect(id, true);
    for (const id in pokemon.turnstatuses) this.addEffect(id, true);
    for (const id in pokemon.movestatuses) this.addEffect(id, true);
  }
  animReset() {
    if (!this.scene.animating) return;
    if (this.$sub) {
  ...
  animSummon(pokemon, slot, instant) {
    ...
    if (this.cryurl) {
      import_battle_sound.BattleSound.playEffect(this.cryurl);
    }
    this.$el.css(this.scene.pos({
      display: "block",
      x: this.x,
  ...
  animTransform(pokemon, useSpeciesAnim, isPermanent) {
    ...
    let sp = import_battle_dex.Dex.getSpriteData(pokemon, this.isFrontSprite, {
      gen: this.scene.gen,
      mod: this.scene.mod
    });
    let oldsp = this.sp;
    if (isPermanent) {
      if (pokemon.volatiles.dynamax) {
        this.oldsp = import_battle_dex.Dex.getSpriteData(pokemon, this.isFrontSprite, {
          gen: this.scene.gen,
          mod: this.scene.mod,
          dynamax: false
        });
    ...
    this.cryurl = sp.cryurl;
    if (!this.scene.animating) return;
    let speciesid = (0, import_battle_dex.toID)(pokemon.getSpeciesForme());
    let doCry = false;
  ...
  clearEffects() {
    for (const id in this.effects) this.removeEffect(id, true);
  ...
  getStatbarHTML(pokemon) {
    ...
    buf += `<strong>${import_battle_log.BattleLog.escapeHTML(ignoreNick ? pokemon.speciesForme : pokemon.name)}`;
    const gender = pokemon.gender;
    if (gender === "M" || gender === "F") {
      buf += ` <img src="${import_battle_dex.Dex.fxPrefix}gender-${gender.toLowerCase()}.png" alt="${gender}" width="7" height="10" class="pixelated" />`;
    }
    ...
    if (symbol) {
      buf += ` <img src="${import_battle_dex.Dex.resourcePrefix}sprites/misc/${symbol}.png" alt="${symbol}" style="vertical-align:text-bottom;" />`;
    }
    if (pokemon.terastallized) {
      buf += ` <img src="${import_battle_dex.Dex.resourcePrefix}sprites/types/Tera${pokemon.terastallized}.png" alt="Tera-${pokemon.terastallized}" style="vertical-align:text-bottom;" height="16" width="16" />`;
    }
  ...
  updateStatbar(pokemon, updatePrevhp, updateHp) {
    ...
    if (pokemon.terastallized) {
      status += `<img src="${import_battle_dex.Dex.resourcePrefix}sprites/types/${encodeURIComponent(pokemon.terastallized)}.png" alt="${pokemon.terastallized}" class="pixelated" /> `;
    } else if (pokemon.volatiles.typechange && pokemon.volatiles.typechange[1]) {
      const types = pokemon.volatiles.typechange[1].split("/");
      for (const type of types) {
        status += '<img src="' + import_battle_dex.Dex.resourcePrefix + "sprites/types/" + encodeURIComponent(type) + '.png" alt="' + type + '" class="pixelated" /> ';
      }
    }
    if (pokemon.volatiles.typeadd) {
      const type = pokemon.volatiles.typeadd[1];
      status += '+<img src="' + import_battle_dex.Dex.resourcePrefix + "sprites/types/" + type + '.png" alt="' + type + '" class="pixelated" /> ';
    }
  ...
  static getEffectTag(id) {
    ...
    if (effect === void 0) {
      let label = `[[${id}]]`;
      if (import_battle_dex.Dex.species.get(id).exists) {
        label = import_battle_dex.Dex.species.get(id).name;
      } else if (import_battle_dex.Dex.items.get(id).exists) {
        label = import_battle_dex.Dex.items.get(id).name;
      } else if (import_battle_dex.Dex.moves.get(id).exists) {
        label = import_battle_dex.Dex.moves.get(id).name;
      } else if (import_battle_dex.Dex.abilities.get(id).exists) {
        label = import_battle_dex.Dex.abilities.get(id).name;
      } else if (import_battle_dex_data.BattleNatures[id.substr(0, 1).toUpperCase() + id.substr(1).toLowerCase()]) {
...
(() => {
  if (!window.Dex || !import_battle_dex.Dex.resourcePrefix) return;
  for (const id in BattleEffects) {
    if (!BattleEffects[id].url) continue;
    BattleEffects[id].url = import_battle_dex.Dex.fxPrefix + BattleEffects[id].url;
  }
})();
...
const BattleOtherAnims = {
  ...
  powerconstruct: {
    anim(scene, [attacker]) {
      ...
      let xf2 = [1, 0, -1, 0];
      let yf2 = [0, 1, 0, -1];
      scene.backgroundEffect("#000000", 1e3, 0.7);
      for (let i = 0; i < 4; i++) {
        scene.showEffect("energyball", {
