...
var battle_tooltips_exports = {};
__export(battle_tooltips_exports, {
  BattleStatGuesser: () => BattleStatGuesser,
  BattleStatOptimizer: () => BattleStatOptimizer,
  BattleTooltips: () => BattleTooltips,
  ModifiableValue: () => ModifiableValue
});
module.exports = __toCommonJS(battle_tooltips_exports);
...
class ModifiableValue {
  constructor(battle, pokemon, serverPokemon) {
    ...
    this.weatherName = this.battle.dex.moves.get(battle.weather).exists ? this.battle.dex.moves.get(battle.weather).name : this.battle.dex.abilities.get(battle.weather).name;
  }
...
class BattleTooltips {
  ...
  static {
    // ms
    this.LONG_CLICK_DELAY = 700;
  }
  static {
    // ms
    this.longTapTimeout = 0;
  }
  static {
    this.elem = null;
  }
  ...
  showTooltip(elem) {
    ...
    switch (type) {
      ...
      case "maxmove": {
        ...
        let gmaxMove = args[3] ? this.battle.dex.moves.get(args[3]) : void 0;
  ...
  getStatusZMoveEffect(move) {
    ...
    let boostText = "";
    if (move.zMove.boost) {
      boostText = Object.entries(move.zMove.boost).map(
        ([stat, boost]) => `${import_battle_text_parser.BattleTextParser.stat(stat)} +${boost}`
      ).join(", ");
    }
    return boostText;
  }
  ...
  showMoveTooltip(move, isZOrMax, pokemon, serverPokemon, gmaxMove) {
    ...
    let ability = (0, import_battle_dex.toID)(serverPokemon.ability || pokemon.ability || serverPokemon.baseAbility);
    ...
    if (isZOrMax === "zmove") {
      if (item.zMoveFrom === move.name) {
        ...
        move = new import_battle_dex_data.Move(move.id, "", {
          ...move,
          name: "Z-" + move.name
        });
        ...
        move = new import_battle_dex_data.Move(zMove.id, zMove.name, {
          ...zMove,
      ...
      if (move.category === "Status") {
        ...
        const basePower = ["gmaxdrumsolo", "gmaxfireball", "gmaxhydrosnipe"].includes(maxMove.id) ? maxMove.basePower : move.maxMove.basePower;
        move = new import_battle_dex_data.Move(maxMove.id, maxMove.name, {
          ...maxMove,
          category: move.category,
          basePower
        });
    ...
    if (categoryDiff) {
      move = new import_battle_dex_data.Move(move.id, move.name, {
        ...move,
        category
      });
    }
    text += `<h2>${move.name}<br />`;
    text += import_battle_dex.Dex.getTypeIcon(moveType);
    text += ` ${import_battle_dex.Dex.getCategoryIcon(category)}</h2>`;
    ...
    if (move.id === "naturepower") {
      ...
      text += `Calls ${import_battle_dex.Dex.getTypeIcon(this.getMoveType(calledMove, value)[0])} ${calledMove.name}`;
    }
    ...
    if (this.battle.hardcoreMode) {
      ...
      if (this.battle.gen === 1 && !(0, import_battle_dex.toID)(this.battle.tier).includes("stadium") && ["recover", "softboiled", "rest"].includes(move.id)) {
        const hpValues = [];
      ...
      if (this.battle.gen === 1 && !(0, import_battle_dex.toID)(this.battle.tier).includes("stadium") && move.id === "substitute") {
  ...
  showPokemonTooltip(clientPokemon, serverPokemon, isActive, illusionIndex) {
    ...
    if (gender === "M" || gender === "F") {
      genderBuf = ` <img src="${import_battle_dex.Dex.fxPrefix}gender-${gender.toLowerCase()}.png" alt="${gender}" width="7" height="10" class="pixelated" /> `;
    }
    let name = import_battle_log.BattleLog.escapeHTML(pokemon.name);
    if (pokemon.speciesForme !== pokemon.name) {
      name += ` <small>(${import_battle_log.BattleLog.escapeHTML(pokemon.speciesForme)})</small>`;
    }
    ...
    if (!illusionIndex || illusionIndex === 1) {
      ...
      text += `<span class="textaligned-typeicons">${types.map((type) => import_battle_dex.Dex.getTypeIcon(type)).join(" ")}</span>`;
      if (pokemon.terastallized) {
        text += `&nbsp; &nbsp; <small>(base: <span class="textaligned-typeicons">${this.getPokemonTypes(pokemon, true).map((type) => import_battle_dex.Dex.getTypeIcon(type)).join(" ")}</span>)</small>`;
      } else if (knownPokemon.teraType && !this.battle.rules["Terastal Clause"]) {
        text += `&nbsp; &nbsp; <small>(Tera Type: <span class="textaligned-typeicons">${import_battle_dex.Dex.getTypeIcon(knownPokemon.teraType)}</span>)</small>`;
      }
      text += `</h2>`;
    }
    if (illusionIndex) {
    ...
    if (pokemon.fainted) {
      ...
      const status = pokemon.status ? ` <span class="status ${pokemon.status}">${pokemon.status.toUpperCase()}</span>` : "";
      text += `<p><small>HP:</small> ${import_battle.Pokemon.getHPText(pokemon, this.battle.reportExactHP)}${exacthp}${status}`;
      if (clientPokemon) {
  ...
  showFieldTooltip() {
    ...
    for (const side of this.battle.sides) {
      ...
      buf += `<td><p class="tooltip-section"><strong>${import_battle_log.BattleLog.escapeHTML(side.name)}</strong>${sideConditions || "<br />(no conditions)"}</p></td>`;
    }
    buf += `</tr><table>`;
    if (!atLeastOne) buf = ``;
  ...
  calculateModifiedStats(clientPokemon, serverPokemon, statStagesOnly) {
    ...
    const ability = (0, import_battle_dex.toID)(
    ...
    let item = (0, import_battle_dex.toID)(serverPokemon.item);
    ...
    if (ability === "klutz" && !speedHalvingEVItems.includes(item) || this.battle.hasPseudoWeather("Magic Room") || clientPokemon?.volatiles["embargo"]) {
      item = "";
    }
    ...
    const speciesName = isTransform && clientPokemon?.volatiles.formechange?.[1] && this.battle.gen <= 4 ? this.battle.dex.species.get(clientPokemon.volatiles.formechange[1]).baseSpecies : species;
    let speedModifiers = [];
  ...
  getSpeedRange(pokemon) {
    ...
    let isRandomBattle = tier.includes("Random Battle") || tier.includes("Random") && tier.includes("Battle") && gen >= 6 || isCGT;
  ...
  getMoveType(move, value, forMaxMove) {
    ...
    if (allowTypeOverride) {
      if (this.battle.rules["Revelationmons Mod"]) {
        ...
        for (let i = 0; i < types.length; i++) {
          if (serverPokemon.moves[i] && move.id === (0, import_battle_dex.toID)(serverPokemon.moves[i])) {
            moveType = types[i];
          }
        }
      }
      ...
      const isSound = !!(forMaxMove ? this.getMaxMoveFromType(moveType, forMaxMove !== true && forMaxMove || void 0) : move).flags["sound"];
      if (isSound && value.abilityModify(0, "Liquid Voice")) {
        moveType = "Water";
      }
    }
    if (move.id === "photongeyser" || move.id === "lightthatburnsthesky" || move.id === "terablast" && pokemon.terastallized || move.id === "terastarstorm" && pokemon.getSpeciesForme() === "Terapagos-Stellar") {
    ...
    if (this.battle.tier.includes("Super Staff Bros")) {
      ...
      if (move.id === "tsignore" && pokemon.getSpeciesForme().startsWith("Meloetta") && pokemon.terastallized) {
        moveType = "Stellar";
      }
      if (move.id === "weatherball" && value.weatherModify(0)) {
  ...
  getMoveAccuracy(move, value, target) {
      ...
      if (value.tryAbility("Hardcore Hustle")) {
        for (let i = 1; i <= 5 && i <= pokemon.side.faintCounter; i++) {
          if (pokemon.volatiles[`fallen${i}`]) {
            value.abilityModify([1, 0.95, 0.9, 0.85, 0.8, 0.75][i], "Hardcore Hustle");
          }
        }
      }
    ...
    if (this.battle.gen === 1 && !(0, import_battle_dex.toID)(this.battle.tier).includes("stadium")) {
  ...
  getMoveBasePower(move, moveType, value, target = null) {
    ...
    let variableBPCap = ["crushgrip", "wringout"].includes(move.id) ? 120 : move.id === "hardpress" ? 100 : void 0;
    ...
    if (move.id === "terrainpulse" && pokemon.isGrounded(serverPokemon)) {
      if (this.battle.hasPseudoWeather("Electric Terrain") || this.battle.hasPseudoWeather("Grassy Terrain") || this.battle.hasPseudoWeather("Misty Terrain") || this.battle.hasPseudoWeather("Psychic Terrain")) {
    ...
    if (move.id === "ragefist") {
      value.set(
        ...
        pokemon.timesAttacked > 0 ? `Hit ${pokemon.timesAttacked} time${pokemon.timesAttacked > 1 ? "s" : ""}` : void 0
      );
    }
    if (!value.value) return value;
    ...
    const allowTypeOverride = !noTypeOverride.includes(move.id) && (move.id !== "terablast" || !pokemon.terastallized);
    if (move.category !== "Status" && allowTypeOverride && !move.isZ && !move.isMax && !move.id.startsWith("hiddenpower")) {
      if (move.type === "Normal") {
    ...
    if (this.battle.hasPseudoWeather("Electric Terrain") && moveType === "Electric" || this.battle.hasPseudoWeather("Grassy Terrain") && moveType === "Grass" || this.battle.hasPseudoWeather("Psychic Terrain") && moveType === "Psychic") {
    ...
    if (pokemon.terastallized && (pokemon.terastallized === move.type || pokemon.terastallized === "Stellar") && value.value < 60 && move.priority <= 0 && !move.multihit && !((move.basePower === 0 || move.basePower === 150) && move.basePowerCallback)) {
    ...
    if (move.id === "steelroller" && !this.battle.hasPseudoWeather("Electric Terrain") && !this.battle.hasPseudoWeather("Grassy Terrain") && !this.battle.hasPseudoWeather("Misty Terrain") && !this.battle.hasPseudoWeather("Psychic Terrain")) {
    ...
    if (this.battle.tier.includes("Super Staff Bros")) {
      if (move.id === "bodycount") {
        value.set(
          ...
          pokemon.side.faintCounter > 0 ? `${pokemon.side.faintCounter} teammate${pokemon.side.faintCounter > 1 ? "s" : ""} KOed` : void 0
        );
      }
  ...
  getItemBoost(move, value, moveType) {
    ...
    let speciesName = isTransform && value.pokemon.volatiles.formechange?.[1] && this.battle.gen <= 4 ? this.battle.dex.species.get(value.pokemon.volatiles.formechange[1]).baseSpecies : species.baseSpecies;
    ...
    if (BattleTooltips.orbUsers[speciesName]?.includes(item.name) && BattleTooltips.orbTypes[item.name]?.includes(moveType)) {
      value.itemModify(1.2);
      return value;
    }
    if (speciesName === "Ogerpon") {
      const speciesForme = value.pokemon.getSpeciesForme();
      if (speciesForme.startsWith("Ogerpon-Wellspring") && itemName === "Wellspring Mask" || speciesForme.startsWith("Ogerpon-Hearthflame") && itemName === "Hearthflame Mask" || speciesForme.startsWith("Ogerpon-Cornerstone") && itemName === "Cornerstone Mask") {
        value.itemModify(1.2);
        return value;
      }
    }
    ...
    if (itemName === "Muscle Band" && move.category === "Physical" || itemName === "Wise Glasses" && move.category === "Special" || itemName === "Punching Glove" && move.flags["punch"]) {
  ...
  getPokemonTypes(pokemon, preterastallized = false) {
    ...
    return pokemon.getTypeList(void 0, preterastallized);
  }
  ...
  getPokemonAbilityText(clientPokemon, serverPokemon, isActive, hidePossible) {
    ...
    if (!text && abilityData.possibilities.length && !hidePossible && !(tier.includes("Almost Any Ability") || tier.includes("Hackmons") || tier.includes("Inheritance") || tier.includes("Metronome"))) {
...
class BattleStatGuesser {
  constructor(formatid) {
    ...
    this.dex = formatid ? import_battle_dex.Dex.mod(formatid.slice(0, 4)) : import_battle_dex.Dex;
    this.ignoreEVLimits = this.dex.gen < 3 || (this.formatid.endsWith("hackmons") || this.formatid.endsWith("bh")) && this.dex.gen !== 6 || this.formatid.includes("metronomebattle") || this.formatid.endsWith("norestrictions");
  ...
  guessRole(set) {
    ...
    let hasMove = {};
    let itemid = (0, import_battle_dex.toID)(set.item);
    let item = this.dex.items.get(itemid);
    let abilityid = (0, import_battle_dex.toID)(set.ability);
    ...
    let moveids = set.moves.map(import_battle_dex.toID);
    ...
    if (bulk < 46e3 && stats.spe >= 70) isFast = true;
    ...
    if (moveCount["Stall"] + moveCount["Support"] / 2 <= 2 && bulk < 135e3 && moveCount[offenseBias] >= 1.5) {
      if (isFast) {
        if (bulk > 8e4 && !moveCount["Ultrafast"]) return "Bulky " + offenseBias + " Sweeper";
    ...
    if (isFast && abilityid !== "prankster") {
      if (stats.spe > 100 || bulk < 55e3 || moveCount["Ultrafast"]) {
  ...
  guessEVs(set, role) {
    ...
    if (this.supportsAVs) {
      ...
      let stat = this.getStat(primaryStat, set, 252, plusStat === primaryStat ? 1.1 : 1);
      let ev = 252;
      while (ev > 0 && stat <= this.getStat(primaryStat, set, ev - 4, plusStat === primaryStat ? 1.1 : 1)) ev -= 4;
      evs[primaryStat] = ev;
      evTotal += ev;
      let secondaryStat = statChart[role][1];
      if (secondaryStat === "hp" && set.level && set.level < 20) secondaryStat = "spd";
      stat = this.getStat(secondaryStat, set, 252, plusStat === secondaryStat ? 1.1 : 1);
      ev = 252;
      while (ev > 0 && stat <= this.getStat(secondaryStat, set, ev - 4, plusStat === secondaryStat ? 1.1 : 1)) ev -= 4;
  ...
  getStat(stat, set, evOverride, natureOverride) {
    ...
    if (evOverride !== void 0) ev = evOverride;
    if (stat === "hp") {
...
function BattleStatOptimizer(set, formatid) {
  ...
  const dex = import_battle_dex.Dex.mod(formatid.slice(0, 4));
  const ignoreEVLimits = dex.gen < 3 || (formatid.endsWith("hackmons") || formatid.endsWith("bh")) && dex.gen !== 6 || formatid.includes("metronomebattle") || formatid.endsWith("norestrictions");
  ...
  const origNature = import_battle_dex_data.BattleNatures[set.nature || "Serious"];
  const origStats = {
  ...
  let savedEVs = 0;
  if (!minusTooLow) {
    for (const stat of import_battle_dex.Dex.statNamesExceptHP) {
