...
var battle_text_parser_exports = {};
__export(battle_text_parser_exports, {
  BattleTextParser: () => BattleTextParser
});
module.exports = __toCommonJS(battle_text_parser_exports);
...
class BattleTextParser {
  ...
  fixLowercase(input) {
    if (this.lowercaseRegExp === void 0) {
  ...
  static effectId(effect) {
    ...
    return (0, import_battle_dex.toID)(effect);
  }
  effect(effect) {
    if (!effect) return "";
    if (effect.startsWith("item:") || effect.startsWith("move:")) {
  ...
  parseArgsInner(args, kwArgs) {
    ...
    switch (cmd) {
      ...
      case "-formechange": {
        ...
        let newSpeciesId = (0, import_battle_dex.toID)(newSpecies);
        let id = "";
      ...
      case "swap": {
        ...
        if (!target || !isNaN(Number(target))) {
          const template2 = this.template("swapCenter");
          return template2.replace("[POKEMON]", this.pokemon(pokemon));
        }
      ...
      case "-start": {
        ...
        if (id === "typechange") {
          const template2 = this.template("typeChange", kwArgs.from);
          return line1 + template2.replace("[POKEMON]", this.pokemon(pokemon)).replace("[TYPE]", arg3).replace("[SOURCE]", this.pokemon(kwArgs.of));
        }
        if (id === "typeadd") {
          const template2 = this.template("typeAdd", kwArgs.from);
          return line1 + template2.replace("[POKEMON]", this.pokemon(pokemon)).replace("[TYPE]", arg3);
        }
        if (id.startsWith("stockpile")) {
          const num = id.slice(9);
          const template2 = this.template("start", "stockpile");
          return line1 + template2.replace("[POKEMON]", this.pokemon(pokemon)).replace("[NUMBER]", num);
        }
        if (id.startsWith("perish")) {
          const num = id.slice(6);
          const template2 = this.template("activate", "perishsong");
          return line1 + template2.replace("[POKEMON]", this.pokemon(pokemon)).replace("[NUMBER]", num);
        }
        if (id.startsWith("protosynthesis") || id.startsWith("quarkdrive")) {
          const stat = id.slice(-3);
          const template2 = this.template("start", id.slice(0, id.length - 3));
          return line1 + template2.replace("[POKEMON]", this.pokemon(pokemon)).replace("[STAT]", BattleTextParser.stat(stat));
        }
      ...
      case "-end": {
        ...
        if (id === "doomdesire" || id === "futuresight") {
          const template2 = this.template("activate", effect);
          return line1 + template2.replace("[TARGET]", this.pokemon(pokemon));
        }
        let templateId = "end";
        let template = "";
      ...
      case "-ability": {
        ...
        if (kwArgs.fail) {
          const template2 = this.template("block", kwArgs.from);
          return line1 + template2;
        }
        if (kwArgs.from) {
          line1 = this.maybeAbility(kwArgs.from, pokemon) + line1;
          const template2 = this.template("changeAbility", kwArgs.from);
          return line1 + template2.replace("[POKEMON]", this.pokemon(pokemon)).replace("[ABILITY]", this.effect(ability)).replace("[SOURCE]", this.pokemon(kwArgs.of));
        }
        const id = BattleTextParser.effectId(ability);
        if (id === "unnerve") {
          const template2 = this.template("start", ability);
          return line1 + template2.replace("[TEAM]", this.team(pokemon.slice(0, 2), true));
        }
        let templateId = "start";
      ...
      case "-item": {
        ...
        if (["thief", "covet", "bestow", "magician", "pickpocket"].includes(id)) {
          const template2 = this.template("takeItem", kwArgs.from);
          return line1 + template2.replace("[POKEMON]", this.pokemon(pokemon)).replace("[ITEM]", this.effect(item)).replace("[SOURCE]", this.pokemon(target || kwArgs.of));
        }
        if (id === "frisk") {
          const hasTarget = kwArgs.of && pokemon && kwArgs.of !== pokemon;
          const template2 = this.template(hasTarget ? "activate" : "activateNoTarget", "Frisk");
          return line1 + template2.replace("[POKEMON]", this.pokemon(kwArgs.of)).replace("[ITEM]", this.effect(item)).replace("[TARGET]", this.pokemon(pokemon));
        }
        if (kwArgs.from) {
          const template2 = this.template("addItem", kwArgs.from);
          return line1 + template2.replace("[POKEMON]", this.pokemon(pokemon)).replace("[ITEM]", this.effect(item));
        }
      ...
      case "-enditem": {
        ...
        const line1 = this.maybeAbility(kwArgs.from, kwArgs.of || pokemon);
        if (kwArgs.eat) {
          const template2 = this.template("eatItem", kwArgs.from);
          return line1 + template2.replace("[POKEMON]", this.pokemon(pokemon)).replace("[ITEM]", this.effect(item));
        }
        const id = BattleTextParser.effectId(kwArgs.from);
        if (id === "gem") {
          const template2 = this.template("useGem", item);
          return line1 + template2.replace("[POKEMON]", this.pokemon(pokemon)).replace("[ITEM]", this.effect(item)).replace("[MOVE]", kwArgs.move);
        }
        if (id === "stealeat") {
          const template2 = this.template("removeItem", "Bug Bite");
          return line1 + template2.replace("[SOURCE]", this.pokemon(kwArgs.of)).replace("[ITEM]", this.effect(item));
        }
        if (kwArgs.from) {
          const template2 = this.template("removeItem", kwArgs.from);
          return line1 + template2.replace("[POKEMON]", this.pokemon(pokemon)).replace("[ITEM]", this.effect(item)).replace("[SOURCE]", this.pokemon(kwArgs.of));
        }
        if (kwArgs.weaken) {
          const template2 = this.template("activateWeaken");
          return line1 + template2.replace("[POKEMON]", this.pokemon(pokemon)).replace("[ITEM]", this.effect(item));
        }
      ...
      case "-status": {
        ...
        const line1 = this.maybeAbility(kwArgs.from, kwArgs.of || pokemon);
        if (kwArgs.from?.startsWith("item:")) {
          const template2 = this.template("startFromItem", status);
          return line1 + template2.replace("[POKEMON]", this.pokemon(pokemon)).replace("[ITEM]", this.effect(kwArgs.from));
        }
        if (BattleTextParser.effectId(kwArgs.from) === "rest") {
          const template2 = this.template("startFromRest", status);
          return line1 + template2.replace("[POKEMON]", this.pokemon(pokemon));
        }
      ...
      case "-curestatus": {
        ...
        if (BattleTextParser.effectId(kwArgs.from) === "naturalcure") {
          const template2 = this.template("activate", kwArgs.from);
          return template2.replace("[POKEMON]", this.pokemon(pokemon));
        }
        const line1 = this.maybeAbility(kwArgs.from, kwArgs.of || pokemon);
        if (kwArgs.from?.startsWith("item:")) {
          const template2 = this.template("endFromItem", status);
          return line1 + template2.replace("[POKEMON]", this.pokemon(pokemon)).replace("[ITEM]", this.effect(kwArgs.from));
        }
        if (kwArgs.thaw) {
          const template2 = this.template("endFromMove", status);
          return line1 + template2.replace("[POKEMON]", this.pokemon(pokemon)).replace("[MOVE]", this.effect(kwArgs.from));
        }
      ...
      case "-singlemove": {
        ...
        if (id === "instruct") {
          const template2 = this.template("activate", effect);
          return line1 + template2.replace("[POKEMON]", this.pokemon(kwArgs.of)).replace("[TARGET]", this.pokemon(pokemon));
        }
      ...
      case "-weather": {
        ...
        if (!weather || weather === "none") {
          const template2 = this.template("end", kwArgs.from, "NODEFAULT");
          if (!template2) return this.template("endFieldEffect").replace("[EFFECT]", this.effect(weather));
          return template2;
        }
        if (kwArgs.upkeep) {
      ...
      case "-activate": {
        ...
        if (id === "lockon" || id === "mindreader") {
          const template2 = this.template("start", effect);
          return line1 + template2.replace("[POKEMON]", this.pokemon(kwArgs.of)).replace("[SOURCE]", this.pokemon(pokemon));
        }
        if ((id === "mummy" || id === "lingeringaroma") && kwArgs.ability) {
          ...
          const template2 = this.template("changeAbility", id);
          return line1 + template2.replace("[TARGET]", this.pokemon(target));
        }
        if (id === "commander") {
          if (target === pokemon) return line1;
          const template2 = this.template("activate", id);
          return line1 + template2.replace("[POKEMON]", this.pokemon(pokemon)).replace(/\[TARGET\]/g, this.pokemon(target));
        }
      ...
      case "-unboost": {
        ...
        if (amount && kwArgs.zeffect) {
          ...
          const template2 = this.template(templateId + "FromItem", kwArgs.from);
          return line1 + template2.replace("[POKEMON]", this.pokemon(pokemon)).replace("[STAT]", BattleTextParser.stat(stat)).replace("[ITEM]", this.effect(kwArgs.from));
        }
      ...
      case "-fail": {
        ...
        const line1 = this.maybeAbility(kwArgs.from, kwArgs.of || pokemon);
        let templateId = "block";
        if (["desolateland", "primordialsea"].includes(blocker) && !["sunnyday", "raindance", "sandstorm", "hail", "snowscape", "chillyreception"].includes(id)) {
      ...
      case "-miss": {
        ...
        const line1 = this.maybeAbility(kwArgs.from, kwArgs.of || pokemon);
        if (!pokemon) {
          const template2 = this.template("missNoPokemon");
          return line1 + template2.replace("[SOURCE]", this.pokemon(source));
        }
