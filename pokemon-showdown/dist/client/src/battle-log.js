...
var battle_log_exports = {};
__export(battle_log_exports, {
  BattleLog: () => BattleLog
});
module.exports = __toCommonJS(battle_log_exports);
...
class BattleLog {
  constructor(elem, scene, innerElem) {
    ...
    if (scene) {
      ...
      this.battleParser = new import_battle_text_parser.BattleTextParser();
    }
  ...
  add(args, kwArgs, preempt, showTimestamps) {
    if (kwArgs?.silent) return;
    const battle = this.scene?.battle;
    if (battle?.seeking) {
      if (battle.stepQueue.length > 2e3) {
        if (battle.seeking === Infinity ? battle.currentStep < battle.stepQueue.length - 2e3 : battle.turn < battle.seeking - 100) {
    ...
    switch (args[0]) {
      ...
      case "c:":
        ...
        if (battle?.ignoreOpponent) {
          if ("\u2605\u2606".includes(rank) && (0, import_battle_dex.toUserid)(name) !== (window.app?.user?.get("userid") || window.PS?.user?.userid)) {
            return;
          }
        }
        const ignoreList = window.app?.ignore || window.PS?.prefs?.ignore;
        if (ignoreList?.[(0, import_battle_dex.toUserid)(name)] && " +^\u2605\u2606".includes(rank)) return;
        let timestampHtml = "";
        if (showTimestamps) {
          const date = timestamp && !isNaN(timestamp) ? new Date(timestamp * 1e3) : /* @__PURE__ */ new Date();
      ...
      case "leave":
      case "l": {
        const user = import_battle_text_parser.BattleTextParser.parseNameParts(args[1]);
      ...
      case "n": {
        const user = import_battle_text_parser.BattleTextParser.parseNameParts(args[1]);
        if ((0, import_battle_dex.toID)(args[2]) === (0, import_battle_dex.toID)(user.name)) return;
        if (!this.lastRename || (0, import_battle_dex.toID)(this.lastRename.to) !== (0, import_battle_dex.toID)(user.name)) {
          this.lastRename = {
            from: args[2],
            to: "",
            element: document.createElement("div")
      ...
      case "unlink": {
        ...
        const user = (0, import_battle_dex.toID)(args[2]) || (0, import_battle_dex.toID)(args[1]);
      ...
      case "hidelines": {
        if (window.PS?.prefs?.nounlink || window.Dex?.prefs?.nounlink) return;
        const user = (0, import_battle_dex.toID)(args[2]);
        this.unlinkChatFrom(user);
        if (args[1] !== "unlink") {
          const lineCount = parseInt(args[3], 10);
      ...
      case "showteam": {
        ...
        const exportedTeam = team.map((set) => {
          let buf = import_battle_teams.Teams.export([set], battle.dex).replace(/\n/g, "<br />");
          if (set.name && set.name !== set.species) {
            buf = buf.replace(set.name, BattleLog.sanitizeHTML(
              `<span class="picon" style="${import_battle_dex.Dex.getPokemonIcon(set.species)}"></span><br />${set.name}`
            ));
          } else {
            buf = buf.replace(
              set.species,
              `<span class="picon" style="${import_battle_dex.Dex.getPokemonIcon(set.species)}"></span><br />${set.species}`
            );
          }
          if (set.item) {
            buf = buf.replace(set.item, `${set.item} <span class="itemicon" style="${import_battle_dex.Dex.getItemIcon(set.item)}"></span>`);
          }
          return buf;
        }).join("");
  ...
  addBattleMessage(args, kwArgs) {
    switch (args[0]) {
      case "warning":
        ...
        if (this.scene) this.scene.wait(1e3);
        return;
      case "variation":
  ...
  addAFDMessage(args, kwArgs = {}) {
    if (!import_battle_dex.Dex.afdMode) return;
    if (!this.battleParser || !this.scene) return;
    const messageFromArgs = (args1, kwArgs1 = {}) => {
      this.messageFromLog(this.battleParser.parseArgs(args1, kwArgs1, true));
    };
    if (args[0] === "move") {
      if (kwArgs.from) return false;
      const moveid = (0, import_battle_dex.toID)(args[2]);
      if (moveid === "taunt") {
        ...
        const battle = this.scene.battle;
        const quote = quotes[((battle.p1.name.charCodeAt(2) || 0) + (battle.p2.name.charCodeAt(2) || 0) * 19 + (battle.p1.name.charCodeAt(3) || 0) * 61 + (battle.p2.name.charCodeAt(3) || 0) + battle.turn + (args[1].charCodeAt(1) || 0) * 109 + (args[1].charCodeAt(2) || 0) * 113) % quotes.length];
      ...
      if (moveid === "chillyreception") {
        const dadJokes = [
          ...
          "Why do they call Ledian the Five Star Pok\xE9mon? I'd give it two stars at best!",
          ...
          "Why should you never play cards with the Cottonweed Pok\xE9mon? Because it always Jump-bluffs!",
          "What Pok\xE9mon comes after Espeon? T-peon!",
          ...
          "What kind of Pok\xE9 Ball do you catch Dustox in? A mothball!",
          ...
          "What's a Hariyama's favorite Pok\xE9mon games? Ultra SuMo!",
          ...
          "That cloud Pok\xE9mon acts innocent, but I know it has an Altaria motive!",
          ...
          "I've always been a fan of ball-shaped Pok\xE9mon, but one in particular Sphealed the deal!",
          "I'd make a joke about Luvdisc, but it's already a joke!",
          "Why is the Iron Leg Pok\xE9mon always covered in dirt and slime? Because it's Meta-gross!",
          "Why is Regirock called the Rock Peak Pok\xE9mon when it's so mid?",
          ...
          "What kind of Pok\xE9 Ball is best for Luxray? A Luxuray Ball!",
          ...
          "What are Aegislash's favorite Pok\xE9mon games? Sword and Shield!",
          ...
          "I heard they buffed Buzzwole in Pok\xE9mon Unite. I don't know why they needed to, it was already buff!",
          ...
          "What Bug/Psychic Pok\xE9mon can be found deep underground embedded in stone? Ore-beetle!",
          ...
          "How does Sirfetch'd pass along all the insider info it knows about new Pok\xE9mon games? It leeks it!",
          ...
          "Plenty of Pok\xE9mon have ridiculous numbers of alternate forms, but Alcremie takes the cake!",
          "Can any Pok\xE9mon besides Tapu Lele set Psychic Terrain? Yes Indeedee!",
          ...
          "Iron Hands is a fantastic Pok\xE9mon. Let's all give it a great big hand!",
          "What should you do if an Iron Treads uses Autotomize? Tread lightly!",
          "What Paradox Pok\xE9mon do you like to run? Personally, I run Thorns!",
          ...
          "How many Pok\xE9mon fans does it take to change a lightbulb? None, they just complain about how much better the last lightbulb was!",
          ...
          "Does Gen 1 have a Pok\xE9mon missing? No!"
        ];
        const battle = this.scene.battle;
        const joke = dadJokes[((battle.p1.name.charCodeAt(2) || 0) + (battle.p2.name.charCodeAt(2) || 0) * 19 + (battle.p1.name.charCodeAt(3) || 0) * 61 + (battle.p2.name.charCodeAt(3) || 0) + battle.turn + (args[1].charCodeAt(1) || 0) * 109 + (args[1].charCodeAt(2) || 0) * 113) % dadJokes.length];
        messageFromArgs(args, kwArgs);
    ...
    if (import_battle_dex.Dex.afdMode !== true) return;
    if (args[0] === "faint") {
      if (!import_battle_dex.Dex.afdFaint) {
        messageFromArgs(args, kwArgs);
        ...
        import_battle_dex.Dex.afdFaint = true;
        return true;
      }
    } else if (args[0] === "-crit") {
      if (!import_battle_dex.Dex.afdCrit) {
        messageFromArgs(args, kwArgs);
        ...
        import_battle_dex.Dex.afdCrit = true;
        return true;
      }
    } else if (args[0] === "move") {
      if (kwArgs.from) return false;
      ...
      if (moveid === "earthquake") {
        ...
        const srNames = [
          ...
          "Secret Solitaire",
          "Sheltered Sand",
          "Surreptitious Sapphire",
          "Undercover Ultramarine"
        ];
  ...
  changeUhtml(id, htmlSrc, forceAdd) {
    id = (0, import_battle_dex.toID)(id);
  ...
  static escapeFormat(formatid = "", fixGen6) {
   ...
   * for the current gen.
   */
  static formatName(formatid = "", fixGen6) {
    if (!formatid) return "";
    let atIndex = formatid.indexOf("@@@");
    if (atIndex >= 0) {
    ...
    if (name.startsWith(`[Gen ${import_battle_dex.Dex.gen}] `)) {
      name = name.slice(`[Gen ${import_battle_dex.Dex.gen}] `.length);
    } else if (name.startsWith(`[Gen ${import_battle_dex.Dex.gen} `)) {
      name = "[" + name.slice(`[Gen ${import_battle_dex.Dex.gen} `.length);
    }
    return name || `[Gen ${import_battle_dex.Dex.gen}]`;
  }
  ...
  static usernameColor(name) {
    ...
    let hash;
    if (import_client_main.Config.customcolors[name]) {
      hash = MD5(import_client_main.Config.customcolors[name]);
    } else {
      hash = MD5(name);
    }
  ...
  parseChatMessage(message, name, timestamp, isHighlighted) {
    ...
    const colorStyle = ` style="color:${BattleLog.usernameColor((0, import_battle_dex.toID)(name))}"`;
    ...
    switch (cmd) {
      ...
      case "mee":
        ...
        if (!showMe) {
          return [
            "chat chatmessage-" + (0, import_battle_dex.toID)(name) + hlClass + mineClass,
      ...
      case "invite":
        let roomid = (0, import_battle_dex.toRoomid)(target);
        return [
          "chat",
          `${timestamp}<em>${clickableName} invited you to join the room "${roomid}"</em><div class="notice"><button class="button" name="joinRoom" value="${roomid}">Join ${roomid}</button></div>`
        ];
      case "announce":
        return [
          "chat chatmessage-" + (0, import_battle_dex.toID)(name) + hlClass + mineClass,
  ...
  static {
    this.interstice = (() => {
      const whitelist = import_client_main.Config.whitelist || [];
      ...
      return {
        ...
        getURI(uri) {
          return `http://${import_client_main.Config.routes.root}/interstice?uri=${encodeURIComponent(uri)}`;
        }
      };
    })();
  }
    ...
    this.tagPolicy = (tagName, attribs) => {
      ...
      if (tagName === "a") {
        ...
        return {
          ...
          attribs: [
            "src",
            `https://player.twitch.tv/?channel=${channelId}&parent=${location.hostname}&autoplay=false`,
            "allowfullscreen",
            "true",
            "height",
            `${height}`,
        ...
        tagName = "strong";
        const color = this.usernameColor((0, import_battle_dex.toID)(getAttrib("name")));
        ...
        return {
          ...
          attribs: [
            ...
            height,
            "src",
            `https://www.youtube.com/embed/${videoId}?enablejsapi=1&playsinline=1${time ? `&start=${time}` : ""}`,
            "frameborder",
            "0",
            "allow",
        ...
        if (iconType) {
          ...
          if (iconType === "pokemon") {
            ...
            unsanitizedStyle = import_battle_dex.Dex.getPokemonIcon(iconValue);
          } else if (iconType === "item") {
            setAttrib("class", "itemicon" + (className ? " " + className : ""));
            unsanitizedStyle = import_battle_dex.Dex.getItemIcon(iconValue);
          } else if (iconType === "type") {
            tagName = import_battle_dex.Dex.getTypeIcon(iconValue).slice(1, -3);
          } else if (iconType === "category") {
            tagName = import_battle_dex.Dex.getCategoryIcon(iconValue).slice(1, -3);
          }
        }
      }
  ...
  static localizeTime(full, date, time, timezone) {
    let parsedTime = /* @__PURE__ */ new Date(date + "T" + time + (timezone || "Z").toUpperCase());
    if (!parsedTime.getTime()) return full;
    let formattedTime;
    if (window.Intl?.DateTimeFormat) {
      formattedTime = new Intl.DateTimeFormat(void 0, {
        month: "long",
        day: "numeric",
        hour: "numeric",
        minute: "numeric"
    ...
    if (replayid) {
      ...
      if (window.Config?.server.id !== "showdown") {
        if (!window.Config?.server.registered) {
          ...
          replayid = import_client_main.Config.server.id + "-" + replayid;
        }
      }
...
`;
    ...
    buf += `<h1 style="font-weight:normal;text-align:center"><strong>${BattleLog.escapeHTML(battle.tier)}</strong><br /><a href="http://${import_client_main.Config.routes.users}/${(0, import_battle_dex.toID)(battle.p1.name)}" class="subtle" target="_blank">${BattleLog.escapeHTML(battle.p1.name)}</a> vs. <a href="http://${import_client_main.Config.routes.users}/${(0, import_battle_dex.toID)(battle.p2.name)}" class="subtle" target="_blank">${BattleLog.escapeHTML(battle.p2.name)}</a></h1>
`;
    ...
    buf += "</div>\n";
    buf += "<script>\n";
    buf += `let daily = Math.floor(Date.now()/1000/60/60/24);document.write('<script src="https://${import_client_main.Config.routes.client}/js/replay-embed.js?version'+daily+'"></'+'script>');
`;
    buf += "<\/script>\n";
    return buf;
  }
...
if (window.Net) {
  (0, import_client_connection.Net)(`/config/colors.json`).get().then((response) => {
    const data = JSON.parse(response);
    Object.assign(import_client_main.Config.customcolors, data);
  }).catch(() => {
  });
}
