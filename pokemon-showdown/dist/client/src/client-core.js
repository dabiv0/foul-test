...
var client_core_exports = {};
__export(client_core_exports, {
  PSBackground: () => PSBackground,
  PSModel: () => PSModel,
  PSStreamModel: () => PSStreamModel,
  PSSubscription: () => PSSubscription
});
module.exports = __toCommonJS(client_core_exports);
 ...
 * @license AGPLv3
 */
const PSURL = `${document.location.protocol !== "http:" ? "https:" : ""}//${import_client_main.Config.routes.client}/`;
...
class PSStreamModel {
  ...
  subscribeAndRun(listener, value = null) {
    const subscription = this.subscribe(listener);
    subscription.listener(value);
    return subscription;
  }
  update(value) {
    if (!this.subscriptions.length && value !== null && value !== void 0) {
...
const PSBackground = new class extends PSStreamModel {
  ...
  load(bgUrl, bgid, menuColors = null) {
    this.id = bgid;
    if (!bgid) {
  ...
  extractMenuColors(bgUrl) {
    ...
    img.onload = () => {
      ...
      if (window.ColorThief) {
        ...
        import_client_main.PS.libsLoaded.then(() => {
          if (changeCount !== PSBackground.changeCount) return;
          this.extractMenuColorsFromImg(img, bgUrl);
        });
      }
  ...
  extractMenuColorsFromImg(img, bgUrl) {
    try {
...
PSBackground.subscribe((bgUrl) => {
  if (!PSBackground.curId) {
    ...
    const buttonStyleElem2 = document.getElementById("mainmenubuttoncolors");
    if (buttonStyleElem2) buttonStyleElem2.textContent = ``;
    return;
  }
  if (bgUrl !== null) {
    let background;
