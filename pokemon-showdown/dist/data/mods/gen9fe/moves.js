...
const Moves = {
  ...
  smackdown: {
    ...
    condition: {
      noCopy: true,
      onStart(pokemon) {
        let applies = !(!(pokemon.hasType("Flying") || pokemon.hasAbility([
          "levitate",
          "airbornearmor",
          "aircontrol",
          "holygrail",
  ...
  mistyterrain: {
    ...
    condition: {
      ...
      onSetStatus(status, target, source, effect) {
        if (!target.isGrounded() || target.isSemiInvulnerable() || ["psn", "tox"].includes(status.id) && source?.hasAbility("miasma")) return;
