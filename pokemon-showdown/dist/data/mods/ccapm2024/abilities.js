...
const Abilities = {
  ...
  outclass: {
    onSourceHit(target, source, move) {
      ...
      if (target !== source && move.category !== "Status" && !source.hasType(targetType) && targetType !== "???" && !(source.volatiles["outclass"] && !source.side.removeSideCondition("fishingtokens"))) {
  ...
  puppetmaster: {
    ...
    onDamagingHitOrder: 1,
    onDamagingHit(damage, target, source, move) {
      if (this.checkMoveMakesContact(move, source, target, true) && this.randomChance(1, 1e3)) {
